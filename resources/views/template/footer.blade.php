@if(!$plus_webview)
<div class="mobile-footer">
    <ul class="foot-nav-info clearfix">
        <li class="foot-home-nav"><a href="{{ Helper::route('home') }}"><span class="iconfont icon-home"></span><div><span class="text">首页</span></div></a></li>
        @if(!empty($session_user) && $session_user['is_vip'] == '1')
        <li><a href="{{ Helper::route('account_center') }}"><span class="iconfont icon-idcard"></span><div><span class="text">
        我的应用</span></div></a></li>
        @else
        <li><a href="{{ Helper::route('article') }}"><span class="iconfont icon-wenzhang"></span><div><span class="text">
        文库</span></div></a></li>
        @endif
        <li>
            <a href="{{ Helper::route('account_store') }}">
                <span class="iconfont icon-dianpu"></span> 
                <div>
                    <span class="text" style="position: relative;">
                        <span>我的店铺</span>
                        <span class="store_order_number u-message_number" style="display:none;"></span> 
                    </span>
                </div>
            </a>
        </li>
        <li>
        	<a href="{{ Helper::route('account_message') }}">
        	   <span class="iconfont icon-message"></span>
        		<div>
        			<span class="text" style="position: relative;">消息
        				<span class="message_number u-message_number" style="display:none;"></span>
        			</span>
        		</div>
        	</a>
        </li>
        <li><a href="{{ Helper::route('account_index') }}"><span class="iconfont icon-user"></span><div><span class="text">账户</span></div></a></li>
    </ul>
</div>
@endif