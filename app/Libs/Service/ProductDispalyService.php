...
class ProductDispalyService
{
    ...
    public static function findProduct($id){
        ...
        foreach ($images as $key => $value) {
            ...
            if($value['type'] == 'description'){
                $description_imgs[] = $value;
            }
        }
        $product['main_imgs'] = $main_imgs;
        $product['description_imgs'] = $description_imgs;
        ...
        $product['self_amount'] = round($product['self_amount'], 2);
        $product['share_amount'] = $max_share_integral * $config['share'];
        $product['share_amount_min'] = $min_share_integral * $config['share'] * 0.95;
        $product['share_amount_max'] = $sku_min_price * 0.37;
        $product['share_amount_min'] = round($product['share_amount_min'], 2);
        if($product['share_amount_min'] > $product['share_amount_max']){
            $product['share_amount_max'] = $product['share_amount_min'] * 1.7;
        }
        $product['share_amount_max'] = round($product['share_amount_max'], 2);
        $product['min_share_integral'] = round($min_share_integral , 2);
        $product['max_share_integral'] = round($max_share_integral , 2);
    ...
    public static function getProductListItem($products){
        ...
        foreach ($products as $key => $product) {
            ...
            if($sku['min_share_integral'] > 0){
                $product['share_amount_min'] = $config['share'] * $sku['min_share_integral'] * 0.95; 
                $product['share_amount_max'] = $sku['price'] * 0.37; 
            ...
            $sku['price_text'] = '￥' . $sku['price'];
            $sku['market_price_text'] = '￥' . $sku['market_price'];
            $product['sku'] = $sku;
            ...
            if($product['is_self'] == '1'){
                ...
                if(!empty($store)){
                    $product['store_name'] = str_limit($store['name'], 16);
                    $product['store_id'] = $store['id'];
                    $product['store_status'] = $store['status'];
                } else {
                    $product['store_name'] = '自营';
                }
            }
    ...
    public static function getSearchProduct($keyword, $sort = null){
        ...
        if($sort == 'sales_numbers'){
            ...
            $products = $products->orderBy('rating', 'desc');
             ...
             $products = $products->leftjoin('product_sku', 'product_sku.product_id', 'product.id')
            ->groupBy('product.id')
            ->orderBy(\DB::raw('min(product_sku.price)'), $sort_order);
        }else {
            $products = $products->orderBy('sales_numbers', 'desc');
            $products = $products->orderBy('created_at', 'desc');
        }
    ...
    public static function getProductReview($product_id, $pageSize, $paginate = false){
        $product_reviews = ReviewsModel::select('user.fullname','user.avatar', 'order_reviews.created_at', 'order_reviews.review_rate', 'order_reviews.review_text', 'order_product.spec', 'order_product.image')
    ...
    public static function getViewdProducts($goods_ids){
        $products = ProductModel::select('product.*')->where('product.enable', '1')
        ->where('product.deleted', '!=', '1')
        ->where('product.is_sale', '1')
        ->where('product.is_gift', '!=', '1')
        ->leftjoin('store_account', 'store_account.user_id', 'product.user_id')
        ->whereRaw("(product.is_self = '1' or  (store_account.status = '2' and store_account.expire_date > now()))")
        ->whereIn('product.id', $goods_ids);
